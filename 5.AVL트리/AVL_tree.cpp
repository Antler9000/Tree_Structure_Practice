...
void AVL_Tree::RemoveTarget(AVL_Node*& pTarget, Stack<AVL_Node*>* pRouteStack)
{
...
void AVL_Tree::ReplaceWithInorderPredecessor(AVL_Node*& pTarget, Stack<AVL_Node*>* pRouteStack)
{
	AVL_Node* pPrevious = NULL;
	AVL_Node* pTraverse = pTarget->m_pLeftChild;
...
void AVL_Tree::ReplaceWithInorderSuccessor(AVL_Node*& pTarget, Stack<AVL_Node*>* pRouteStack)
{
	AVL_Node* pPrevious = NULL;
	AVL_Node* pTraverse = pTarget->m_pRightChild;
	pRouteStack->Push(pTarget);
...
void AVL_Tree::BalancingAllTargetToRoot(Stack<AVL_Node*>* pRouteStack)
{
	while (pRouteStack->IsEmpty() == false)
	{
		AVL_Node* pRetraverse = pRouteStack->Pop();
		AVL_Node* pParentOfRetraverse = pRouteStack->GetTop();
...
void AVL_Tree::BalancingTargetNode(AVL_Node* pTarget, AVL_Node* pParent)
{
	int leftHeight = 0;
	int rightHeight = 0;
...
void AVL_Tree::LL_Rotation(AVL_Node* pTarget, AVL_Node* pParent)
{
	cout << "LL 회전" << endl;
...
void AVL_Tree::LR_Rotation(AVL_Node* pTarget, AVL_Node* pParent)
{
	cout << "LR 회전" << endl;

	AVL_Node* pLR_Location = pTarget->m_pLeftChild->m_pRightChild;
...
void AVL_Tree::RL_Rotation(AVL_Node* pTarget, AVL_Node* pParent)
{
	cout << "RL 회전" << endl;

	AVL_Node* pRL_Location = pTarget->m_pRightChild->m_pLeftChild;
...
void AVL_Tree::RR_Rotation(AVL_Node* pTarget, AVL_Node* pParent)
{
	cout << "RR 회전" << endl;
...
void AVL_Tree::Insert(int newKey, int newData)
{
	if (m_pHead == NULL)
	{
		m_pHead = new AVL_Node(newKey, newData);
		return;
	}

	AVL_Node* pTraverse = m_pHead;
	Stack<AVL_Node*> pRouteStack;
	while (true)
	{
		if (newKey < pTraverse->m_key)
		{
			...
			{
				pTraverse->m_pLeftChild = new AVL_Node(newKey, newData);
		...
		else
		{
			...
			{
				pTraverse->m_pRightChild = new AVL_Node(newKey, newData);
				pRouteStack.Push(pTraverse);
				BalancingAllTargetToRoot(&pRouteStack);
				return;
			}
